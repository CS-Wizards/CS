...
POST 와 PUT
POST는 데이터를 생성하고 PUT은 데이터를 수정한다.

 

하지만 PUT의 경우 해당 리소스가 없다면 생성할수 있다.
...
{ "id" : 2, "name" : "Lee", "age" : 20 }
{ "id" : 3, "name" : "Lee", "age" : 20 }
```
POST로 같은 정보를 3번 보내면 해당 data가 3번 생성된다,
...
//결과
{ "id" : 4, "name" : "Lee", "age" : 20 }
```
하지만 PUT의 경우에는 몇번을 보내도 생성되는것은 한번이다.

즉 POST는 멱등하지 않지만 PUT은  멱등하다.
